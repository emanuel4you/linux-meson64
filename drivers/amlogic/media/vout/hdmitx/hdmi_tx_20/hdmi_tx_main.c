...
{
	unsigned int i, j;
	int hlg = hdev->rxcap.hdr_sup_eotf_hlg;
	int hdr10 = hdev->rxcap.hdr_sup_eotf_smpte_st_2084;
	if (hdev->hdr10_support == 0)
		hdr10 = 0;
	else if (hdev->hdr10_support == 1)
		hdr10 = 1;
	if (hdev->hlg_support == 0)
		hlg = 0;
	else if (hdev->hlg_support == 1)
		hlg = 1;
...
static unsigned long prev_mode_change_time;
static unsigned long prev_drm_packet_change_timed;

static int set_disp_mode_auto_int(int force)
{
	int ret =  -1;
	...
	if (!force && (vic_ready != HDMI_Unknown) && (vic_ready == vic)) {
...
static inline int set_disp_mode_auto(void)
{
	return set_disp_mode_auto_int(0);
}
...
static void hdmitx_sdr_hdr_uevent(struct hdmitx_dev *hdev)
{
	...
	if (check_mode) {
		const struct vinfo_s *info = hdmitx_get_current_vinfo();
		if (info && strstr(info->name, "2160") && time_before(prev_mode_change_time, prev_drm_packet_change_timed))
			set_disp_mode_auto_int(1);
	}
}
...
static void hdmitx_set_drm_pkt(struct master_display_info_s *data)
{
	...
	int hdr10 = hdev->rxcap.hdr_sup_eotf_smpte_st_2084;
	int hlg = hdev->rxcap.hdr_sup_eotf_hlg;
	if (hdev->hdr10_support == 1)
		hdr10 = 1;
	else if (hdev->hdr10_support == 0)
		hdr10 = 0;
	if (hdev->hlg_support == 1)
		hlg = 1;
	else if (hdev->hlg_support == 0)
		hlg = 0;
	...
	prev_drm_packet_change_timed = jiffies;
	...
	struct device_attribute *attr, char *buf)
{
	...
	pos += snprintf(buf+pos, PAGE_SIZE, "HDR10 support: %d\n", hdev->hdr10_support);
	pos += snprintf(buf+pos, PAGE_SIZE, "HLG support: %d\n", hdev->hlg_support);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (strncmp(buf, "unplug_powerdown", 16) == 0) {
		...
		if (count > 6) {
			int set_disp_mode = 1;
			if (strncmp(buf+3, "10-2", 4) == 0 && hdmitx_device.hdr10_support != 2) {
				hdmitx_device.hdr10_support = 2;
				pr_info("HDR10 support by EDID\n");
			}
			else if (strncmp(buf+3, "10-0", 4) == 0 && hdmitx_device.hdr10_support != 0) {
				hdmitx_device.hdr10_support = 0;
				pr_info("HDR10 support forced off\n");
			}
			else if (strncmp(buf+3, "10-1", 4) == 0 && hdmitx_device.hdr10_support != 1) {
				hdmitx_device.hdr10_support = 1;
				pr_info("HDR10 support forced on\n");
			}
			else
				set_disp_mode = 0;
			if (set_disp_mode)
		...
		if (count > 4) {
			int set_disp_mode = 1;
			if (strncmp(buf+3, "-2", 2) == 0 && hdmitx_device.hlg_support != 2) {
				hdmitx_device.hlg_support = 2;
				pr_info("HLG support by EDID\n");
			}
			else if (strncmp(buf+3, "-0", 2) == 0 && hdmitx_device.hlg_support != 0) {
				hdmitx_device.hlg_support = 0;
				pr_info("HLG support forced off\n");
			}
			else if (strncmp(buf+3, "-1", 2) == 0 && hdmitx_device.hlg_support != 1) {
				hdmitx_device.hlg_support = 1;
				pr_info("HLG support forced on\n");
			}
			else
				set_disp_mode = 0;
			if (set_disp_mode)
			 ...
			 int *ppos, char *buf)
{
	...
	for (j = 0; j < 3; j++) {
		if (prxcap->RxAudioCap[i].cc3 & (1 << j))
			*ppos += snprintf(buf + *ppos - 1, PAGE_SIZE, "%s/",
				aud_sample_size[j+1]);
	}

	if (*ppos != pos)
		*ppos += snprintf(buf + *ppos - 2, PAGE_SIZE, " bit\n") - 2;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < prxcap->AUD_count; i++) {
		...
		case CT_ATRAC:
		case CT_ONE_BIT_AUDIO:
			pos += snprintf(buf + pos - 1, PAGE_SIZE,
				"MaxBitRate %dkHz\n",
				prxcap->RxAudioCap[i].cc3 * 8) - 1;
			break;
		case CT_DOLBY_D:
		case CT_DTS_HD:
		case CT_MAT:
		case CT_DST:
			pos += snprintf(buf + pos - 1, PAGE_SIZE, "DepVaule 0x%x\n",
				prxcap->RxAudioCap[i].cc3) - 1;
			break;
		case CT_WMA:
		default:
	...
	struct device_attribute *attr, char *buf)
{
	...
	int hdr10 = prxcap->hdr_sup_eotf_smpte_st_2084;
	int hlg = prxcap->hdr_sup_eotf_hlg;
	if (hdmitx_device.hdr10_support == 1)
		hdr10 = 1;
	else if (hdmitx_device.hdr10_support == 0)
		hdr10 = 0;
	if (hdmitx_device.hlg_support == 1)
		hlg = 1;
	else if (hdmitx_device.hlg_support == 0)
		hlg = 0;
...
static int amhdmitx_probe(struct platform_device *pdev)
{
	...
	hdmitx_device.hlg_support = hdmitx_device.hdr10_support = 2; /* detect by EDID */
...
static void check_hdmiuboot_attr(char *token)
{
	...
	for (i = 0; cd[i] != NULL; i++) {
		if (strstr(token, cd[i])) {
			strlcat(attr, cd[i], sizeof(attr));
			strlcpy(hdmitx_device.fmt_attr, attr,
				sizeof(hdmitx_device.fmt_attr));
			break;
		}
	}
